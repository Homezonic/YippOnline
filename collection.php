...
if ((!(isset($_GET["page"]))) || (intval($_GET["page"]) <= 0)) {
	header('location:collection?id=' . $_GET["id"] . '&page=1');
	exit();
}
...
if ($_GET["page"] > $numPages) {
	header('location:collection?id=' . $collectionID . '&page=' . $numPages);
	exit();
}
$gridOffset = ($page - 1) * $gridLimit;
$gridStart = $gridOffset + 1;
$gridEnd = min(($gridOffset + $gridLimit), $numProducts);
$prevPage = ($page > 1) ? 'collection?id=' . $collectionID . '&page=' . ($page - 1) : "#";
$nextPage = ($page < $numPages) ? 'collection?id=' . $collectionID . '&page=' . ($page + 1) : "#";
$sqlQuery = "SELECT *
FROM `products`
INNER JOIN `brands`
...
<main>
	<div class="top-banner">
		<div class="featured-image-gradient opacity-mask d-flex align-items-center" data-opacity-mask="rgba(0, 77, 218, 0.1)">
			<div class="container">
				<div class="breadcrumbs">
					<ul>
						<li><a type="button">Collections</a></li>
		...
		<img src="images/grid/collection-<?php echo $collectionID;?>.jpg" class="img-fluid">
	</div>
			...
			<div class="col-lg-9">
				...
				<div class="pagination-wrapper">
					<ul class="pagination">
						...
						<li>
							<a href="<?php echo 'collection?id='. $collectionID . '&page=' . $i; ?>" <?php echo ($i == $page) ? "class='active'" : "" ?>><?php echo $i; ?></a>
						</li>
						<?php } ?>
