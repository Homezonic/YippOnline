...
if ((!(isset($_GET["page"]))) || (intval($_GET["page"]) <= 0)) {
	header('location:category?id=' . sprintf("%02d", $_GET["id"]) . '&page=1');
	exit();
}
...
if ($_GET["page"] > $numPages) {
	header('location:category?id=' . sprintf("%02d", $categoryID) . '&page=' . $numPages);
	exit();
}
$gridOffset = ($page - 1) * $gridLimit;
$gridStart = $gridOffset + 1;
$gridEnd = min(($gridOffset + $gridLimit), $numProducts);
$prevPage = ($page > 1) ? 'category?id=' . sprintf("%02d", $categoryID) . '&page=' . ($page - 1) : "#";
$nextPage = ($page < $numPages) ? 'category?id=' . sprintf("%02d", $categoryID) . '&page=' . ($page + 1) : "#";
$sqlQuery = "SELECT *
FROM `products`
INNER JOIN `brands`
...
<main>
	<div class="top-banner">
		...
		<img src="images/grid/category-<?php echo $categoryID;?>.jpg" class="img-fluid">
	</div>
			...
			<div class="col-lg-9">
				...
				<div class="pagination-wrapper">
					<ul class="pagination">
						...
						<li>
							<a href="<?php echo 'category?id='. sprintf("%02d", $categoryID) . '&page=' . $i; ?>" <?php echo ($i == $page) ? "class='active'" : "" ?>><?php echo $i; ?></a>
						</li>
						<?php } ?>
