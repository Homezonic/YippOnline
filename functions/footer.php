		...
		<div class="row">
			<div class="col-lg-3 col-md-6">
				...
				<div class="collapse dont-collapse-sm links" id="collapse-1">
					<ul>
						...
						<li><a href="contact">Contact Retail Store</a></li>
						<li><a href="about">About Retail Store</a></li>
					</ul>
				</div>
			</div>
			<div class="col-lg-3 col-md-6">
			...
			<div class="col-lg-3 col-md-6">
				...
				<div class="collapse dont-collapse-sm contacts" id="collapse-3">
					<ul>
						<li><i class="ti-home"></i><a href="https://www.google.com/maps/search/?api=1&query=Ikeja+Lagos" target="_blank" rel="noreferrer">Ikeja, Lagos</a></li>
						<li><i class="ti-headphone-alt"></i><a href="tel://+23058038524">+230 5803 8524</a></li>
						<li><i class="ti-email"></i><a href="mailto:info@yipp.onlin">info@yipp.onlin</a></li>
					</ul>
				</div>
			</div>
			<div class="col-lg-3 col-md-6">
				<h3 data-bs-target="#collapse-4">Subscribe to Retail Store's newsletter</h3>
		...
		<div class="row add-bottom-25">
					...
					<li>
						<div class="styled-select lang-selector">
							<select id="selectLanguage" onchange="var currentDate = new Date(); var currentTime = currentDate.getTime(); var expireTime = currentTime + 31556952000; currentDate.setTime(expireTime); document.cookie = 'LANGUAGE=' + this.value + '; expires=' + currentDate.toUTCString(); location.reload()">
								<option value="English" <?php if ($_COOKIE['LANGUAGE'] == 'English') {
								    echo 'selected';
								} ?>>English</option>
								<option value="French" <?php if ($_COOKIE['LANGUAGE'] == 'French') {
								    echo 'selected';
								} ?>>Français</option>
								<option value="Yoruba" <?php if ($_COOKIE['LANGUAGE'] == 'Yoruba') {
								    echo 'selected';
								} ?>>Kreol</option>
							</select>
						</div>
					...
					<li>
						<div class="styled-select currency-selector">
							<select id="selectCurrency" onchange="var currentDate = new Date(); var currentTime = currentDate.getTime(); var expireTime = currentTime + 31556952000; currentDate.setTime(expireTime); document.cookie = 'CURRENCY=' + this.value + '; expires=' + currentDate.toUTCString(); location.reload()">
								<option value="MUR" <?php if ($_COOKIE['CURRENCY'] == 'MUR') {
								    echo 'selected';
								} ?>>Mauritian Rupee (MUR)</option>
								<option value="USD" <?php if ($_COOKIE['CURRENCY'] == 'USD') {
								    echo 'selected';
								} ?>>United States Dollar (USD)</option>
								<option value="EUR" <?php if ($_COOKIE['CURRENCY'] == 'EUR') {
								    echo 'selected';
								} ?>>Euro (EUR)</option>
								<option value="NGN" <?php if ($_COOKIE['CURRENCY'] == 'NGN') {
								    echo 'selected';
								} ?>>Naira (NGN)</option>
							</select>
						</div>
			...
			<div class="col-lg-6">
				<ul class="additional-links">
					...
					<li><a href="https://jits.ng" target="_blank" rel="noreferrer" onclick="alert('Hello World!')">Akande Joshua &copy; Retail Store 2023</a></li>
				</ul>
			</div>
		</div>
	</div>
