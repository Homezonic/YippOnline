...
if (empty($firstName) || empty($address) || empty($city) || empty($zip) || empty($country) || empty($phone)) {
    ...
    $outputMsg = '<h6 class="text-error">ZIP code must have 6 digits</h6>';
} elseif (!(preg_match($validNumeric, $phone))) {
    $outputMsg = '<h6 class="text-error">Invalid phone</h6>';
} elseif (strlen($phone) !== 11) {
    $outputMsg = '<h6 class="text-error">Phone must have 11 digits</h6>';
} elseif ($phone[0] !== '0') {
    $outputMsg = '<h6 class="text-error">Phone must start with 0</h6>';
...
$response = [
    ...
    'currentPhone'   => '+234 ' . substr($_SESSION['phone'], 0, 4) . ' ' . substr($_SESSION['phone'], 4, 4),
];
echo json_encode($response);
