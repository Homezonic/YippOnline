...
if ((strcmp($outputMsg, 'quick-add-to-cart-success') == 0) || (strcmp($outputMsg, 'already-in-cart') == 0)) {
    if (($_COOKIE['CURRENCY'] == 'EUR') && (isset($_SESSION['MURtoEUR'])) && (isset($_SESSION['MURtoNGN']))) {
        $response = [
            'outputMsg'          => $outputMsg,
        ...
        $response = [
            'outputMsg'          => $outputMsg,
            'formattedProductID' => sprintf('%06d', $productID),
            'formattedNewPrice'  => '₦' . number_format($newPrice * $_SESSION['MURtoNGN']),
            'formattedOldPrice'  => '₦' . number_format($oldPrice * $_SESSION['MURtoNGN']),
            'numCartItems'       => $numCartItems,
        ];
    } else {
        $response = [
